...
function sanitizeFilename(originalName, fallbackExt = "") {
  ...
  const safeBase =
    slugify(base, { lower: true, strict: true, trim: true }) || "file";
...
function createSecureUpload({
          ...
          if (allowImages && isImage) {
            ...
            try {
              ...
              if (meta.width < imageMinWidth || meta.height < imageMinHeight) {
                return next({
                  status: 400,
                  message: `Image too small. Min ${imageMinWidth}x${imageMinHeight}px.`,
                });
              }
              if (meta.width > imageMaxWidth || meta.height > imageMaxHeight) {
                return next({
                  status: 400,
                  message: `Image too large. Max ${imageMaxWidth}x${imageMaxHeight}px.`,
                });
              }
              ...
              if (
                ...
                return next({
                  status: 400,
                  message: "Animated GIFs are not allowed.",
                });
              }
            } catch (e) {
              return next({
  ...
  function uploadArray(fieldName, count = maxFiles) {
    return [
      ...
      validateUploadedFiles(fieldName, { array: true }),
    ];
  }
